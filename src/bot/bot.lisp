...
(defmethod initiate-communities ((moonbot moonbot))
  (mapc (lambda (community)
          (initiate-filters community))
        (communities moonbot))
  (initiate-spellcheckers moonbot))

(defmethod start ((moonbot moonbot))
...
primary listening thread. If Luna is not logged in then evaluates (login <luna>) and recalls this function."
  (unless (found-modules moonbot)
  ...
  (if (logged-in-p moonbot)
      (progn (psetf (stopp moonbot) nil
             ...
             moonbot)
      (progn (login moonbot)
             ;;               (initiate-communities moonbot)
             (start moonbot))))

(defmethod stop ((moonbot moonbot))
  (setf (stopp moonbot) t)
  ...
  (let ((err-count 0))
    (handler-bind ((moonbot-still-running
      ...
      (wait-for-stop moonbot)))
  (log:info "Stopped")
  (setf (stopp moonbot) nil))

(defmethod force-stop ((moonbot moonbot))
...
interrupt the main execution thread and cause Luan to stop."
  ...
  (ignore-errors
   (bt:interrupt-thread (thread moonbot)
                        (lambda () (throw 'bail nil)))))

(defmethod wait-for-stop ((moonbot moonbot))
...
moonbot to get done processing its current batch of messages. This method will
...
second 'wait-60' recalls 'wait-for-stop' making it wait another 60 seconds."
  (let ((x 0))
    (restart-case
        (loop :while (bt:thread-alive-p (thread moonbot))
              :do (sleep 5)
                  (incf x 5)
